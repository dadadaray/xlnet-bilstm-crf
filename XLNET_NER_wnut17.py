...
class DataProcessor(object):
    ...
    def _read_data(cls, input_file,delimiter='\t'):
        ...
        with open(input_file) as stream:
            ...
            labeled_tokens = [zip(*g) for k, g in groupby(reader, lambda x: not [s for s in x if s.strip()]) if not k]
            tokens, labels = zip(*labeled_tokens)
            ts=[list(t) for t in tokens]
            ls=[list(l) for l in labels]
            ...
            print("numbers of sentences", len(ts))
            return lines

class NerProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, "emerging.train.conll.preproc.url")), "train"
        )

    def get_dev_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, "emerging.dev.conll.preproc.url")), "dev"
        )

    def get_test_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, "emerging.test.conll.preproc.url")), "test"
        )
    def get_labels(self):
        """
        ...
        :return:
        """
        return [ "<pad>", "B-location", "I-location", "B-group","I-group", "B-corporation","I-corporation", "B-person", "B-creative-work", "B-product","I-product", "I-person","I-creative-work", "O", "X",
                "<cls>", "<sep>"]
...
class XLNetModelBuilder(object):
    ...
    def _create_model(self,
        ...
        for (i, label) in enumerate(label_list):
            label_map[label] = i
        with open(FLAGS.middle_output + "/label2id1.pkl", 'wb') as w:
       ...
       #  #添加BILSTM_CRF
        ...
        #max_seq_length = embedding.shape[0].value

        #print("embedding_shape",max_seq_length)
       #
       #  # 算序列真实长度
...
def main(_):
    ...
    if FLAGS.do_train:
        ...
        print("aaaaaaaaaaaaaaaaaaaaaaaaathis is time")
        print(end - start)
    ...
    if FLAGS.do_predict:
        with open(FLAGS.middle_output + '/label2id1.pkl', 'rb') as rf:
            label2id1 = pickle.load(rf)
            id2label = {value: key for key, value in label2id1.items()}
